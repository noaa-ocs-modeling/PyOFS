...
if __name__ == '__main__':
    ...
    contour_names = [
        f'{letter}{number}' for number in range(1, 5) for letter in ['A', 'B', 'C']
    ]
    ...
    for velocity_product in velocity_products:
        ...
        with fiona.open(input_path, layer=layer_name) as contours_file:
            contour_names = sorted(
                numpy.unique([feature['properties']['contour'] for feature in contours_file])
            )
            ...
            for contour_name in contour_names:
                ...
                contour_records = [
                    ...
                    for record in filter(
                        lambda record: record['properties']['contour'] == contour_name,
                        contours_file,
                    )
                ]

                for record in contour_records:
                    contour_datetime = datetime.strptime(
                        record['properties']['datetime'], '%Y-%m-%dT%H:%M:%S'
                    )
                ...
                values[contour_name][velocity_product] = pandas.DataFrame(
                    {
                        'datetime': contour_datetimes,
                        'area': contour_areas,
                        'perimeter': contour_perimeters,
                    }
                )

    plotting_values = {
        'area': '%' if plot_percentages else 'm^2',
        'perimeter': '%' if plot_percentages else 'm',
    }
    ...
    for velocity_product in velocity_products:
        ...
        for plotting_value, plotting_unit in plotting_values.items():
            ...
            colors = dict(
                zip(
                    (contour_name[1] for contour_name in contour_names),
                    pyplot.cm.viridis(numpy.linspace(0, 1, 4)),
                )
            )
            ...
            for color_index, (contour_name, contour_values) in enumerate(
                ...
                y_values = (
                    contour_values[plotting_value] / starting_value if plot_percentages else 1
                )
                (line,) = axis.plot(
                    contour_values['datetime'], y_values, '-o', label=contour_name, color=color
                )
                axis.annotate(
                    contour_name,
                    xy=(1, line.get_ydata()[-1]),
                    xytext=(6, 0),
            ...
            axis.axhline(
                y=1 if plot_percentages else starting_value, linestyle=':', color='k', zorder=0
            )

            figure.savefig(
                plot_dir / f'{velocity_product}_{plotting_value}.pdf',
